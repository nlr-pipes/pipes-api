...
class SpatialDimensions(BaseModel):
    ...
    extent: str = Field(
        title="extent",
        description="""
            The spatial extent of the tool/dataset, can be general
            (e.g. regional, continental, utility service area, ect.) or
            specific (e.g. CONUS, ERCOT, ect.).
        """,
    )
    fidelity: str = Field(
        title="fidelity",
        description="""
            The spatial resolution/fidelity of the tool/dataset. Can be general
            (zonal, nodal, high voltage, distribution voltage, ect.) or specific
            (e.g. balancing areas, states, specific voltages).
        """,
    )
    scope: list[str] | None = Field(
        ...
        default=None,
        description="""
            The spatial scope of the tool/dataset (Generation, Transmission,
            Distribution, Device or Asset Level, Other).
        """,
    )
...
class GeneralDataDescriptionSchema(BaseModel):
    ...
    schema_info: DataSchemaSchema | None = Field(
        title="schema_info",
        default=None,
        description="Information on data schema.",
    )
    temporal_dimensions: TemporalDimensions | None = Field(
        title="temporal_dimensions",
        default=None,
        description="Temporal characteristics of the dataset.",
    )
    spatial_dimensions: SpatialDimensions | None = Field(
...
class DatasetSchema(BaseModel):
    # The dataset requirement can be a specific published dataset (2024 ATB)
    # or a general data requirement (Load Forecasts)
    # Required
    name: str = Field(
        title="name",
        description="Name of the dataset.",
...
class ToolMaturity(BaseModel):
    ...
    publication_history: bool = Field(
        ...
        description="""
            Are there peer-reviewed publications describing innovation and
            disemination of grid science and technology that this tool has supported?
        """,
    )
    external_validation_documented: bool = Field(
        title="external_validation_documented",
        description="""
            Is there external validation via a publication, e.g., via comparison to
            results from other methodologies, real-world events, or replicated
            results from external users?
        """,
    )
    application: bool = Field(
        title="application",
        description="""
            Has this tool resulted in considerations used in deployment decisions,
            e.g., the analysis informed TA recipient decisions?
        """,
    )
    ...
    secure_independent_usage: bool = Field(
        ...
        description="""
            Can a user run the tool independently in a commonly accessible, secure
            environment (e.g. locally)? N/A if the tool is not intended to be run
            by an external user.
        """,
    )
    ...
    accessible_for_external_users: bool = Field(
        ...
        description="""
            Can this tool be transfered to an external user for independent use?
            N/A if the tool is not intended to be run by an external user.
        """,
    )
...
class Input(BaseModel):
    general_data_description: GeneralDataDescriptionSchema | None = Field(
        title="general_data_description",
        default=None,
        description="""
            The general data description output describes known data produced by
...
class ConfigSchema(BaseModel):
    # Optional
    model_modes: list[str] | None = Field(
        title="model_modes",
        default=None,
        description="Different modes of the tool.",
    )
    model_options: dict[str, str] | None = Field(
        title="model_options",
        default=None,
    ...
    example_configs: list[str] | None = Field(
...
class IFACCatalogModelCreate(BaseModel):
    ...
    prime_organization: str | None = Field(
    ...
    software_type: str | None = Field(
        ...
        default=None,
        description="""
            DOE Code software type (e.g. 'Open Source, Publicly Available Repository
            (OS-PAR), Open Source, No Publicly Available Repository (OS-NPAR),
            Closed Source (CS)).
        """,
        # TODO: Add options
    )
    ...
    maturity: ToolMaturity = Field(
        title="maturity",
        description="""
            Required tool maturity information for different aspects of the tool's
            maturity for use in IFAC TA scoping.
        """,
    )
    teams: list[Teams] = Field(
        title="teams",
        description="""
            Team(s) that use/develop/maintain the tool. For tools with multiple
            organizations, add multiple entries.
        """,
    )
    expected_scenarios: list[ExpectedScenario] | None = Field(
        title="expected_scenarios",
        description="Default/expected scenarios the tool runs.",
    )
    config: ConfigSchema | None = Field(
        title="config",
        default=None,
    ...
    requirements: Requirements = Field(
        title="requirements",
        description="""
            Tool requirements. Spatial, temporal, environment, and misc blocks.
            Each supports multiple entries via list.
        """,
    )
...
class IFACCatalogModelUpdate(IFACCatalogModelCreate):
    ...
    maturity: ToolMaturity | None = Field(
        title="maturity",
        default=None,
        description="""
            Required tool maturity information for different aspects of the tool's
    ...
    # --- PIPES System Fields ---
    other: dict | None = Field(  # TODO: does this belong here or does user interact?
        title="other",
        default={},
        description="other metadata info about the model in dictionary",
    )
